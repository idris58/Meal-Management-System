...
export function MealProvider({ children }: { children: ReactNode }) {
  ...
  const addMember = (name: string, role: Role) => {
    setMembers([...members, { id: uuidv4(), name, role, deposit: 0, mealsEaten: 0, isActive: true, avatar: name.substring(0, 2).toUpperCase() }]);
  };
  ...
  const addExpense = (amount: number, description: string, type: 'meal' | 'fixed', paidBy: string) => {
    setExpenses([...expenses, { id: uuidv4(), amount, description, type, date: new Date().toISOString(), paidBy }]);
  };
  ...
  const logMeal = (memberId: string, count: number, date: string) => {
    ...
    setMealLogs(prev => {
      ...
      if (existingIdx >= 0) {
        ...
        else newLogs[existingIdx] = { ...newLogs[existingIdx], count };
        return newLogs;
      }
